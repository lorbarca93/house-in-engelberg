...
This document describes in detail how the **Model Sensitivity** (deterministic) and **MC Sensitivity** (probabilistic) analyses are calculated in the Engelberg Property Investment Simulation.
...
6. [MC Sensitivity (Probabilistic)](#mc-sensitivity-probabilistic)
...
| Analysis | Type | What it varies | What it measures |
|----------|------|----------------|------------------|
| **Model Sensitivity – Equity IRR** | Deterministic | 15 parameters (low / base / high) | Equity IRR (15-year) and Monthly After-Tax Cash Flow |
| **Model Sensitivity – Cash-on-Cash** | Deterministic | Same 15 parameters | Year 1 cash flow ÷ initial equity |
| **Model Sensitivity – Monthly NCF** | Deterministic | Same 15 parameters | Annual cash flow per owner ÷ 12 |
| **MC Sensitivity** | Probabilistic | 5 parameters × 10 values each | NPV > 0 probability at each value |
...
MC Sensitivity **fixes one parameter at a chosen value** and runs a full **Monte Carlo** simulation (many random scenarios) to estimate the probability that NPV > 0; this is repeated for 10 values per parameter.
...
3. For each of the **15 parameters** (12 from config + 3 special):
...
### Parameters Tested (12 config + 3 special)

**Config parameters** (modified via `apply_sensitivity` on `BaseCaseConfig`):

| Key | Display Name | Low Factor | High Factor | Clamp Min | Clamp Max | Notes |
|-----|--------------|------------|-------------|-----------|-----------|--------|
| `maintenance_rate` | Maintenance Reserve Rate | 0.0 | 2.0 | 0 | — | 0% to 2× base |
...
| `amortization` | Amortization Rate | 0.0 | 2.0 | 0 | — | 0% to 2× base |
...
- `low_value = base_value × low_factor`  
- `high_value = base_value × high_factor`  
...
- **Property appreciation**: `low = base × 0.6`, `high = base × 1.4` (clamp 0–100%).
- **Inflation**: `low = base × 0.50`, `high = base × 1.50` (clamp min 0).
- **Selling costs**: `low = base × 0.743589`, `high = base × 1.256410` (clamp min 5%).
...
- **Formula**: `(Year 1 cash flow per owner / equity per owner) × 100`.
- **Steps**:
  1. `compute_annual_cash_flows(config)`.
...
## MC Sensitivity (Probabilistic)

MC Sensitivity measures how **NPV > 0 probability** changes when **one** input is fixed at different values. For each value we run a full Monte Carlo simulation and count the fraction of runs with NPV > 0.
...
3. **10 points**: linear spacing between `min_value` and `max_value`:
   - `value[i] = min_value + (max_value − min_value) × i / 9` for `i = 0, …, 9`.

Implemented in `generate_parameter_range()` in `engelberg/mc_sensitivity.py`.

### Probability Calculation

For **each** of the 10 values of **each** of the 5 parameters:
...
2. Run **Monte Carlo** with that config:
   - `run_monte_carlo_simulation(modified_config, num_simulations=..., use_lhs=True, use_parallel=False)` (parallel is disabled when MC Sensitivity runs parameter values in parallel).
...
Sensitivity analyses never mutate the base config; they build **new** configs with **one** (or in MC Sensitivity, one at a time) parameter changed. That is done by `apply_sensitivity()` in `engelberg/core.py`.
...
- **Expenses**: New `ExpenseParams` with overrides for management fee, cleaning cost per stay, length of stay, insurance (derived from `insurance_rate × purchase_price` if `insurance_rate` is set), maintenance rate, and property value (set to new purchase price if `purchase_price` is set).

So for Model Sensitivity, “low” and “high” scenarios are exactly **base config with one parameter replaced** by the scaled low/high value; for MC Sensitivity, each of the 10 values per parameter is used the same way.

---

## Summary

- **Model Sensitivity** uses **deterministic** low/base/high (from factors and clamps) and recomputes **one** metric (Equity IRR, Cash-on-Cash, or Monthly NCF) per scenario; impact = |high − low|; 15 parameters (12 config + 3 projection-only).
- **MC Sensitivity** uses **10 evenly spaced values** per parameter, runs a **Monte Carlo** at each value, and records **NPV > 0 probability**; impact = max_prob − min_prob; 5 parameters.
- Both rely on **single-parameter overrides** via `apply_sensitivity()` and the same core cash flow and projection functions; the only difference is whether the metric is a single deterministic number (IRR, CoC, NCF) or a probability from many stochastic runs (MC Sensitivity).
...
- `engelberg/mc_sensitivity.py` — parameter ranges, value generation, MC runs, probability and impact.
- `engelberg/mc_sensitivity_ranges.py` — MC parameter definitions.
